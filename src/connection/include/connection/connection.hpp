...
struct connection {
    ...
    auto async_send_message(message& message, CompletionToken&& token)
    {
        ...
        return boost::asio::async_compose<
            ...
            [this, &message, state = states::header](
                auto& self, const boost::system::error_code& ec = {},
                std::size_t n = 0) mutable {
                switch (state) {
                    case header:
                        state = states::body;
                        boost::asio::async_write(
                            socket,
                            boost::asio::buffer(&message.header,
                    ...
                    case body:
                        ...
                        if (!ec) {
                            boost::asio::async_write(
                                socket,
                                boost::asio::buffer(
                                    message.bytes.data() + message.cursor,
                                    message.size()),
                                std::move(self));
                        } else {
                            self.complete(ec, 0);
                ...
                switch (state) {
                    ...
                    case body:
                        ...
                        if (!ec) {
                            ...
                            boost::asio::async_read(
                                socket, boost::asio::buffer(dest.bytes),
                                std::move(self));
                        } else {
                            self.complete(ec, 0);
                        }
